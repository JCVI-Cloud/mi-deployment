...
rsync --stats -avx --exclude /root/.bash_history --exclude /home/*/.bash_history --exclude /etc/ssh/ssh_host_* --exclude /etc/ssh/moduli --exclude /etc/udev/rules.d/*persistent-net.rules --exclude /var/lib/ec2/* --exclude=/mnt/* --exclude=/proc/* --exclude=/tmp/* --exclude /root/.ssh/* --exclude /home/ubuntu/.ssh/* --exclude /var/lib/rabbitmq/mnesia / $IMAGE_MOUNT_POINT

# Because we're using xfs as the root file system, edit /etc/fstab on the image to reflect so
echo "rsych complete; updating $IMAGE_MOUNT_POINT/etc/fstab"
...
# Clear out log files
echo "Clearing out any log files from $IMAGE_MOUNT_POINT/var/log"
